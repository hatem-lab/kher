<?php

return [

    'hello' => 'Your Welcome in Kher Admin Panel',
    'Active' => 'Active',
    'Inactive' => 'Inactive',
    'Name' => 'الاسم',
    'Actions' => 'الإجراءات',
    'Block' => 'حظر',
    'Activate' => 'تفعيل',
    'Edit' => 'تعديل',
    'Delete' => 'حذف',
    'Search' => 'بحث',
    'Search..' => 'بحث..',
    'User_Updated_Successfully' => 'تم تعديل المسؤول بنجاح',
    'Operation_Failed' => 'العملية فشلت!',
    'delete_warning' => 'هل تريد حذف هذا العنصر؟',
    'option_delete_warning' => 'هل تريد حذف هذا الخيار؟',


    'New' => 'جديد',
    'Old' => 'قديم',

    'Add' => 'إضافة',


    'Hide' => 'إخفاء',

    'view' => 'استعراض',
    'hidden' => 'Hidden',
    'in_review' => 'In Review',
    'published' => 'Published',
    'canceled' => 'Canceled',



    'Canceled' => 'Canceled',


    'Remove' => 'Remove',


    'Certificate_Deleted_Successfully' => 'تم حذف الشهادة بنجاح',
    'Diploma_Deleted_Successfully' => 'تم حذف شهادة الدبلوم بنجاح',
    'Course_Deleted_Successfully' => 'تم حذف الكورس بنجاح',
    'Lecture_Deleted_Successfully' => 'تم حذف المحاضرة بنجاح',
    'Homework_Deleted_Successfully' => 'تم حذف الواجب بنجاح',
    'Test_Deleted_Successfully' => 'تم حذف الاختبار بنجاح',
    'Question_Deleted_Successfully' => 'تم حذف السؤال بنجاح',
    'Survey_Deleted_Successfully' => 'تم حذف الاستبيان بنجاح',
    'Student_Deleted_Successfully' => 'تم حذف الطالب بنجاح',
    'User_Deleted_Successfully' => 'تم حذف المستخدم بنجاح',
    'Teacher_Deleted_Successfully' => 'تم حذف المدرس بنجاح',
    'Role_Deleted_Successfully' => 'تم حذف السماحية بنجاح',

    'Title' => 'العنوان',
    'Choose_one' => 'اختر',

    'Student_Account_Deleted_Successfully' => 'تم حذف الطالب بنجاح',
    'Student_Account_Accepted_Successfully' => 'تم قبول الطالب بنجاح',


    'student_activate_warning' => 'هل تريد تفعيل حساب هذا الطالب؟ ',
    'student_block_warning' => 'هل تريد إلغاء تفعيل حساب هذا الطالب؟ ',

    'user_activate_warning' => 'هل تريد تفعيل حساب هذا المستخدم؟ ',
    'user_block_warning' => 'هل تريد إلغاء تفعيل حساب هذا المستخدم؟ ',

    'teacher_activate_warning' => 'هل تريد تفعيل حساب هذا المدرس؟ ',
    'teacher_block_warning' => 'هل تريد إلغاء تفعيل حساب هذا المدرس؟ ',

    'absent_warning' => 'هل تريد تغيير حالة هذا الطالب الى غياب؟',
    'present_warning' => 'هل تريد تغيير حالة هذا الطالب الى حضور؟',

    'Finish' => 'إنهاء',
    'course_finish_warning' => 'هل تريد إنهاء هذا الكورس؟',
    'course_activate_warning' => 'هل تريد تفعيل هذا الكورس؟',

    'Status_Changed_Success' => 'تم تغيير الحالة بنجاح',

    'File_Deleted_Successfully' => 'تم حذف الملف بنجاح',
    'Blog_Accepted_Successfully'=>'تم قبول المدونة بنجاح',
    'Blog_Rejected_Successfully'=>'تم رفض المدونة بنجاح',
];
