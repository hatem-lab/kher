...
@section('title',trans('surveys/surveys.survey details'))
            ...
            <div class="row">
                <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-10 offset-md-1">
                    <div class="breadcrumb-title">
                        <h2>{{trans('surveys/surveys.survey details')}}</h2>
                        <ul class="breadcrumb-menu">
                            <li><a href="{{route('site.home')}}">{{trans('tests/tests.Home')}} </a></li>
                            <li><a href="{{route('student.survay')}}">{{trans('surveys/surveys.surveys')}} </a></li>
                            <li>{{$survay->title}}</li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="row gx-5">
                <div class="col-lg-8">
                    <div class="course-details">
                        <div class="course-details-img">
                            <img src="{{asset('end-user/assets/img/survey.png')}}" alt="Image">
                        </div>
                        <ul class="course-details-meta">
                            <li><i class="las la-clock"></i>{{trans('tests/tests.Date')}}
                                : {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $survay->created_at)->format('d-m-Y')}}
                            </li>
                            <li><i class="las la-graduation-cap"></i>{{trans('tests/tests.Time')}}
                                : {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $survay->created_at)->format('H:i')}}
                            </li>
                ...
                <div class="col-lg-4">
                    <div class="course-details-widget sidebar-box">
                        <ul>
                            @if(isset($survay->course))
                                <li>
                                    <p><i class="las la-user-graduate"></i>{{trans('tests/tests.Diploma')}}</p>
                                    <p>{{$survay->course->diploma->title}}</p>
                                </li>
                                <li>
                                    <p><i class="las la-file-alt"></i>{{trans('tests/tests.Course')}}</p>
                                    <p>{{$survay->course->title}}</p>
                                </li>
                            @else
                                <li>
                                    <p><i class="las la-user-graduate"></i>{{trans('tests/tests.type')}}</p>
                                    <p>{{trans('surveys/surveys.general')}}</p>
                                </li>
                            @endif
                            ...
                            <li>
                                ...
                                <p>{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $survay->created_at)->format('d-m-Y')}}</p>
                            </li>
                            <li>
                                <p><i class="las la-question-circle"></i>{{trans('tests/tests.Time')}}</p>
                                <p>{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $survay->created_at)->format('H:i')}}</p>
                            </li>


                        </ul>
                    </div>
                <!-- <a href="{{route('student.survay.questions', $survay->id)}}" class="btn v3">{{trans('tests/tests.apply_now')}}</a> -->
                    <div class="course-enroll-widget sidebar-box">

                        @if(in_array($survay->id, $student_surveys) )
                            <div class="single-course-price">
                                {{trans('surveys/surveys.survey Done')}}
                            </div>
                        @else
                            <a href="{{route('student.survay.questions', $survay->id)}}"
